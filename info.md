...
- **🧼 Reset Service**: Call `tnb_calculator.reset_storage` and type `RESET` to clear cached totals, historical months, and cached holidays when you want a fresh start.
- **📅 Holiday Diagnostics**: `sensor.tnb_cached_holidays_count` lists every cached holiday and the last fetch timestamp in its attributes for easy verification.
- **✅ Configuration Guardrails**: Setup validates energy entities and Calendarific API keys with useful error messages.
